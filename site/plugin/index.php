...
// NEW VERSION (to have each project/exhibition name followed by the title of the section it belongs to) //		
foreach($pages as $reord)
{
$order[$reord['sec_desc']][] = array(
'id' => $reord['id'],
...
'disp' => $reord['sec_disp']);
	...
	foreach($order as $key => $out)
	{
		...
		//	$s .= "<li$active><a href='" . BASEURL . ndxz_rewriter($page['url']) . "' onclick=\"do_click();\">" . $page['title'] . "</a></li>\n";
			$s .= "<li$active><a href='" . BASEURL . ndxz_rewriter($page['url']) . "' onclick=\"do_click();\">" . $page['title'] . "</a> <span class='mathieuisasuperfreak'>$page[secdesc]</span></li>\n";
		}
